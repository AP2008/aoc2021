...
pub fn runner() -> (usize, usize) {
    ...
    for line in input.lines() {
        let k = line.split(" | ").collect::<Vec<_>>()[0]
            .split(' ')
            .map(|n| n.trim().chars().collect())
            .collect();
        let n = line.split(" | ").collect::<Vec<_>>()[1]
            .split(' ')
            .map(|n| n.trim().chars().collect())
            .collect();
        s += part2(k, n);
...
pub fn a2i(s: &char) -> usize {
    (s.to_digit(17).unwrap() - 10) as usize
}

fn part2(signal_ps: Vec<Vec<char>>, output_vals: Vec<Vec<char>>) -> usize {
    let mut seg_7 = vec!['e'; 7];
    ...
    seg_7[a2i(v3
        .iter()
        .find(|x| !elem4.contains(x) && !sized[3][0].contains(x))
        .unwrap())] = 'g';
    let mut s = 0;
    for t in output_vals {
        let mut n = Vec::new();
        for k in t {
            n.push(seg_7[a2i(&k)].to_string());
        }
        n.sort_unstable();
        s *= 10;
        s += DIGITS[&n.join("")];
